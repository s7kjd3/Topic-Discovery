...
def lstm_words():
   ...
   # x4 = LSTM(75, return_sequences=True,name='lstm_4')(x3)
    ...
    sequence_autoencoder.fit(data, embedding_layer.predict(data), epochs=10,callbacks=[LearningRateScheduler(lr_schedule) ])
    # Training is done
...
def lstm_sentences():
    ...
    sequence_autoencoder.fit(sent_back, sent_back, epochs=10,callbacks=[LearningRateScheduler(lr_schedule)])#, callbacks=[tbCallBack])
    print encoder.predict(sent_back)

    csvname = 'Amazon_lstm_hierachical_autoencoder_representation_test'
...
def write_vec_to_csv(doc_vector_train, targets, filenames, csvname):
    ...
    with open('reviews_30_100_' + csvname + '_.csv', 'w') as f:
