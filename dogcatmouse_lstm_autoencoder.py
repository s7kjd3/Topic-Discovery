...
def get_data():
    path =['/home/flippped/thesis_project/dogcatmouse']
    data = []
    filename = []
...
def get_corpus():
    ...
    #filter empty docs
    corpus_train, data,  filenames = filter_docs(corpus_train_tmp,data, filename, lambda doc: (len(doc) != 0))
    # for i in corpus_train:
    #     print i
    return corpus_train,filenames
def filter_docs(corpus, texts,  filenames, condition_on_doc):
    """
...
def preprocess_embedding():
    corpus_train,  filenames = get_corpus()
    print corpus_train
    tokenizer = Tokenizer()
    ...
    return data,filenames,embedding_matrix, word_index

def lstm():
    data, filenames, embedding_matrix, word_index = preprocess_embedding()
    EMBEDDING_DIM = 300
    MAX_SEQUENCE_LENGTH = 500
    ...
    # [ batchsize, timesteps, input_dimension ]
    ...
    write_vec_to_csv(encoded_data,filenames,csvname)
...
def write_vec_to_csv(doc_vector_train,filenames, csvname):
    ...
    with open('dogcatmouse_' + csvname + '_.csv', 'w') as f:
