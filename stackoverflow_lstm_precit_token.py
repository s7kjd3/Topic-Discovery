...
def lstm():
    ...
    print data
    uniques, ids = np.unique(data, return_inverse=True)
   ...
   # x4 = LSTM(75, return_sequences=True,name='lstm_4')(x3)
    ...
    pred = Dense(len(uniques),activation='softmax')(decoded)
    sequence_autoencoder = Model(sequence_input, pred)
    ...
    sequence_autoencoder.fit(data, targets, epochs=50, callbacks=[tbCallBack])
    # Training is done
